...
export default async (req) => {
  ...
  try {
    ...
    const isFirstUserTurn = (userCount === 1 && assistantCount === 1); // přesně první dialog po úvodním bublině v UI

    async function callOpenAI(msgs) {
      if (!OPENAI_API_KEY) {
        // Bez klíče vrátíme originál (nebude překlad)
    ...
    async function translateToUserLanguage(text, userMsgs) {
      ...
      const msgs = [
        { role: "system", content: "Detect user's language from the sample and translate the provided text fully into that language. Keep formatting and meaning. If already matching, return as is." },
    ...
    // ---------- Onboarding texty (EN jako zdroj; vždy přeložíme do jazyka uživatele) ----------
...
Here you can see a **3D visualization** of our hotel:
...
`.trim(),
    ...
    // ---------- 1) Onboarding: jen při první uživatelské zprávě ----------
    if (isFirstUserTurn) {
      const bundle = onboardingBundleEN();
      const translated = await translateToUserLanguage(bundle, messages);
    ...
    // (Volitelné: na ne-parkovací intent teď nevracíme nic dlouhého, onboarding už proběhl.)
    ...
    // ---------- 3) PARKOVÁNÍ ----------
    // Jedna noc → odvodíme rozsah z data příjezdu (pokud uvedeno)
    ...
    // načíst dostupnost pro každou noc
    ...
    if (intent === "parking" && effectiveRange) {
      ...
      const lines   = out.map(d => d.ok ? `• ${d.date}: volno ${d.free} / ${d.total}${d.note ? ` (${d.note})` : ""}` : `• ${d.date}: dostupnost neznámá${d.note ? ` (${d.note})` : ""}`);
...
${allFree
  ...
  : anyFull
    ...
    // extrakce detailů pro rychlý zápis
    function extractDetails(msgs){
      const t = lastUserText().trim(); if (!t) return null;
      const timeMatch = t.match(/(\b\d{1,2}[:.]\d{2}\b)/);
      const arrival = timeMatch ? timeMatch[1].replace('.',':') : null;
      const parts = t.split(/[,;\n]/).map(s=>s.trim()).filter(Boolean);
      let plate=null;
      for (const p of parts){ const c = p.replace(/\s+/g,''); if (/^[A-Za-z0-9-]{5,}$/.test(c)){ plate=c.toUpperCase(); break; } }
      let name=null;
    ...
    // zapsat rezervaci, když vše sedí
    if (AVAILABILITY && AVAILABILITY.allFree && AVAILABILITY.nights>0 && details && details.guest_name && details.car_plate){
      ...
      if (!failed){
        ...
        const instr  = await translateToUserLanguage(packCZ, messages);
        return ok(
...
${instr}`
    ...
    if (intent === "parking"){
      const ask = parseDatesStrict(userText).ask;
      return ok(ask || "Napište prosím termín parkování ve formátu **DD.MM.–DD.MM.YYYY**.");
    }

    // Nic konkrétního – už onboarding NEPOSÍLÁME
    return ok(await translateToUserLanguage("How can I help you further? (Wi-Fi, taxi, parking, AC, power…)", messages));
